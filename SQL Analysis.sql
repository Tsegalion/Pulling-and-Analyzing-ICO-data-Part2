...
SELECT category.category_name, SUM(raised_usd) AS Total_raised
FROM icos
JOIN category
ON icos.categoryid = category.categoryid
WHERE raised_usd IS NOT NULL
GROUP BY 1
ORDER BY 2 DESC
LIMIT 10
...
SELECT platform.platforn_name, SUM(raised_usd) AS Total_raised
FROM icos
JOIN platform
ON icos.platformid = platform.platformid
WHERE raised_usd IS NOT NULL
GROUP BY 1
ORDER BY 2 DESC
LIMIT 10
...
SELECT token_role.token_role, SUM(raised_usd) AS Total_raised
FROM icos
JOIN token_role
ON icos.token_roleid = token_role.token_roleid
WHERE raised_usd IS NOT NULL
GROUP BY 1
ORDER BY 2 DESC -- Utility tokens are the most common
...
FROM icos
JOIN date ON icos.dateid = date.dateid
WHERE raised_usd IS NOT NULL
GROUP BY 1
ORDER BY 3 DESC -- Q2 raised the highest and also recorded the highest number of projects
...
FROM icos
WHERE raised_usd IS NOT NULL AND fundraise_goal IS NOT NULL


-- Create a temp table of ICO projects who raised above 10 million dollars
CREATE TEMP TABLE BigICOs AS
SELECT project.project_name, date.start_date, date.end_date, category.category_name, platform.platforn_name, total_token, ico_price, fundraise_goal, raised_usd, sold
FROM icos
JOIN project ON icos.projectid = project.projectid
JOIN platform ON icos.platformid = platform.platformid
JOIN category ON icos.categoryid = category.categoryid
JOIN date ON icos.dateid = date.dateid
WHERE raised_usd >= 10000000
AND ico_price IS NOT NULL
AND total_token IS NOT NULL
AND fundraise_goal IS NOT NULL
AND sold IS NOT NULL

-- ICO projects who raised above 10 million dollars
SELECT *
FROM BigICOs

-- Was the amount raised was based of the total tokens sold during the ICO
SELECT sold_range, COUNT(sold_range) AS Total
FROM(
SELECT *,
	CASE WHEN sold < 10 THEN 'Less than 10%'
	WHEN sold BETWEEN 10 AND 25 THEN '10% - 25%'
	WHEN sold BETWEEN 25 AND 50 THEN '25% - 50%'
	WHEN sold BETWEEN 50 AND 75 THEN '50% - 75%'
	WHEN sold BETWEEN 75 AND 100 THEN '75% - 100%' END AS Sold_range
FROM BigICOs) AS T1
GROUP BY 1
...
SELECT Price_range, COUNT(Price_range) AS Total
FROM(
SELECT *,
	CASE WHEN ico_price < 0.01 THEN 'Less than $0.01'
	...
	WHEN ico_price > 1000.00 THEN 'Above $1k' END AS Price_range
FROM BigICOs) AS T1
GROUP BY 1
